...
public class QKART_Tests {
    ...
    public void TestCase01(@Optional("testUser") String TC1_Username, @Optional("abc@123") String TC1_Password) throws InterruptedException {
        ...
        // onTestStart("onStart method started");
    ...
    public void TestCase02() throws InterruptedException {
        ...
        assertTrue(status, "User Registration");
    ...
    public void TestCase03() throws InterruptedException {
        ...
        for (WebElement webElement : searchResults) {
            ...
            if (!elementText.toUpperCase().contains("YONEX")) {
                //logStatus("TestCase 3", "Test Case Failure. Test Results contains un-expected values: " + elementText,
                        //"FAIL");
                // return false;
                assertTrue(status, "Test Case Failure. Test Results contains un-expected values");
            }
        }

        //logStatus("Step Success", "Successfully validated the search results ", "PASS");
        assertTrue(status, "Successfully validated the search results ");
        ...
        if (searchResults.size() == 0) {
            if (homePage.isNoResultFound()) {
                ...
                assertFalse(status, "Successfully validated that no products found message is displayed");
            }
            // logStatus("TestCase 3", "Test Case PASS. Verified that no search results were found for the given text",
            //         "PASS");
            assertFalse(status, "Test Case PASS. Verified that no search results were found for the given text");
        } else {
            //logStatus("TestCase 3", "Test Case Fail. Expected: no results , actual: Results were available", "FAIL");
            //return false;
            assertTrue(status, "Test Case Fail. Expected: no results , actual: Results were available");
        }
    ...
    public void TestCase04() throws InterruptedException {
        ...
        for (WebElement webElement : searchResults) {
            ...
            if (result.verifySizeChartExists()) {
                ...
                assertTrue(status, "Successfully validated presence of Size Chart Link");

                // Verify if size dropdown exists
                status = result.verifyExistenceofSizeDropdown(driver);
                //logStatus("Step Success", "Validated presence of drop down", status ? "PASS" : "FAIL");
                assertTrue(status, "Validated presence of drop down");
                ...
                if (result.openSizechart()) {
                    ...
                    if (result.validateSizeChartContents(expectedTableHeaders, expectedTableBody, driver)) {
                        ...
                        assertFalse(status, "Failure while validating contents of Size Chart Link");
                    }
    ...
    public void TestCase11() throws InterruptedException {
        ...
        assertTrue(status, "Verify that Advertisement 2 is clickable");
